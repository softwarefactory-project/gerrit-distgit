...
function install_bazel() {
    curl -L --output bzl_install.sh https://github.com/bazelbuild/bazel/releases/download/${BAZEL_VERSION}/bazel-${BAZEL_VERSION}-installer-linux-x86_64.sh
    chmod +x bzl_install.sh
    sudo ./bzl_install.sh
}
...
function gerrit_install() {
    ...
    popd
    cp gerrit-${VERSION}-${RELEASE}.el7.tar.gz artifacts/
}
